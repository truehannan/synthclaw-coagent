...
export function getProjectRoot() {
  ...
  const commonPaths = [
    join(process.env.HOME || "/root", "conclave"),
    join(process.env.HOME || "/root", "conclave-coagent"),
    "/opt/agent",
    "/root/conclave",
    "/root/conclave-coagent",
  ];
...
export function generateEnvContent() {
  ...
  lines.push("");
  // Use actual project root for CONCLAVE_BASE_DIR (not the config default)
  // This ensures Python finds agent.log, agent.db, workspace/ in the right place
  const actualRoot = getProjectRoot();
  lines.push(`CONCLAVE_BASE_DIR=${actualRoot}`);
